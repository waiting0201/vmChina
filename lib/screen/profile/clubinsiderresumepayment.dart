...
class ClubinsiderResumePayment extends StatefulWidget {
  final BrandMemberPlan brandmemberplan;
  final Brand brand;
  final String membershipfeedid;
  const ClubinsiderResumePayment({
  ...
  @override
  State<ClubinsiderResumePayment> createState() =>
      _ClubinsiderResumePaymentState();
}

class _ClubinsiderResumePaymentState extends State<ClubinsiderResumePayment> {
  ...
  void initState() {
    ...
    if (_authChangeProvider.status && _member.awid != null) {
      ...
      if (_member.membershipfees!
          .any((e) => e.membershipfeeid == widget.membershipfeedid)) {
        var membershipfee = _member.membershipfees!
            .singleWhere((e) => e.membershipfeeid == widget.membershipfeedid);
        var expiredate = DateTime.parse(membershipfee.expiredate!);
        _trial = expiredate.difference(today).inDays > 0
            ? expiredate.difference(today).inDays
            : 0;
      }
    }
  }
                      ...
                      onPressed: () {
                        showDialog(
                          ...
                          if (value) {
                            ...
                            httpService
                                .postresumeautorenew(
                              _authChangeProvider.member.memberid,
